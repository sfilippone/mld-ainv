subroutine mld_dsparse_orthbase(alg,n,a,p,z,nzrmax,sp_thresh,info)
  ...
  case (mld_ainv_orth1_)
    call mld_dsparse_orth_1(n,a,p,zcsc,nzrmax,sp_thresh,info)
    if (info /= 0) goto 9999
...
!!$    ! !$         & p(1) = 1.d-3 
!!$
!!$    call psb_ensure_size(n+1, z%icp,  info)
!!$    call psb_ensure_size(n*nzrmax, z%ia,  info)
!!$    call psb_ensure_size(n*nzrmax, z%val, info)
!!$    ! 
!!$    !
!!$    z%descra  = 'GUN'
!!$    z%fida    = 'CSC'
...
!!$             & nzrz,z%ia(ipzz:ipzz+nzrz-1),z%val(ipzz:ipzz+nzrz-1))
!!$! !$        write(psb_err_unit,*) '     ..........: ',nzrz,z%ia(ipzz:ipzz+nzrz-1),z%val(ipzz:ipzz+nzrz-1)
...
!!$               & alpha, nzzi, z%ia(ipzi:ipzi+nzzi-1), z%val(ipzi:ipzi+nzzi-1),&
!!$               & done, nzzj, z%ia(ipzj:ipzj+nzzj-1), z%val(ipzj:ipzj+nzzj-1),&
!!$               & info)
!!$
...
!!$          if (info == psb_success_) call psb_ensure_size(ipzn+nzrz-1,znew%aspk,info)
!!$          znew%ia1(ipzn:ipzn+nzrz-1)  = z%ia(ipzj:ipzj+nzzj-1)
!!$          znew%aspk(ipzn:ipzn+nzrz-1) = z%val(ipzj:ipzj+nzzj-1)
!!$          znew%ia2(j+1) = ipzn + nzrz
!!$        end if
...
!!$      end do
!!$      if (i < n) then 
!!$        ! !$        write(psb_err_unit,*)  i,'Copy step ', z%icp(i+1:i+2),':',znew%ia2(i+1:i+2)
!!$        z%icp(i+1:i+2) = znew%ia2(i+1:i+2)
!!$        ipzj = z%icp(i+1)
!!$        nzzj = z%icp(i+2) - ipzj
!!$        z%ia(ipzj:ipzj+nzzj-1)  = znew%ia1(ipzj:ipzj+nzzj-1)
!!$        z%val(ipzj:ipzj+nzzj-1) = znew%aspk(ipzj:ipzj+nzzj-1) 
!!$        ! !$        write(psb_err_unit,*)  i,'Copy step ', z%ia(ipzj:ipzj+nzzj-1), z%val(ipzj:ipzj+nzzj-1) 
