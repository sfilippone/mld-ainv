...
!!$contains
  subroutine mld_dsparse_orthbase(alg,n,a,p,z,nzrmax,sp_thresh,info)
    ...
    case (mld_ainv_orth2_) 
      call mld_dsparse_orth_2(n,a,p,zcsc,nzrmax,sp_thresh,info)
!!$    if (info /= 0) goto 9999
    ...
    do i = 1, n
      ...
      do j=i, n
        ...
        p(j) = psb_spdot_srtd(nzra,a%ja(ipza:ipza+nzra-1),a%val(ipza:ipza+nzra-1),&
             & nzrz,zmat%cols(j)%idx(1:nzrz),zmat%cols(j)%val(:nzrz))
      end do
      if (p(i) == dzero) then 
      ...
      do j=i+1,n 
        ...
        if (abs(alpha) > sp_thresh) then 
          ...
          call psb_nspaxpby(nzrz,iz,valz,&
               & alpha, nzzi, zmat%cols(i)%idx(1:nzzi), zmat%cols(i)%val(1:nzzi),&
               & done, nzzj, zmat%cols(j)%idx(1:nzzj), zmat%cols(j)%val(1:nzzj),&
               & info)
          ...
          zmat%cols(j)%nz          = nzrz
          zmat%cols(j)%idx(1:nzrz) = iz(1:nzrz)
          zmat%cols(j)%val(1:nzrz) = valz(1:nzrz)
        end if
      end do
      if (info /= psb_success_) then 
        info = psb_err_internal_error_
